...
export const metadata: Metadata = {
  title: "TradeMind vs TradeZella — Best Trading Journal for Prop Firm Traders (2025)",
  description: "Honest comparison: TradeMind vs TradeZella. See which trading journal gives funded traders the edge they need — real-time tilt detection, multi-account dashboards, and prop firm drawdown tracking.",
  openGraph: {
    title: "TradeMind vs TradeZella (2025 Comparison)",
...
const ROWS = [
  { feature: "Daily mental check-in (mood, sleep, stress)", tm: true, tz: true, note: "" },
  { feature: "GO / CAUTION / NO-TRADE verdict", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Real-time tilt detection (consecutive loss alert)", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Confluence combination analytics (FVG+OB win rate)", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Multi-prop-firm account dashboard", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Drawdown usage tracker per account", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "ICT / SMC setup tagging (FVG, OB, BOS, MSS…)", tm: true, tz: true, note: "" },
  { feature: "Execution quality score (SL discipline, profit capture)", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Monte Carlo equity projection (P10/P50/P90)", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "CSV import (Tradovate, NinjaTrader, Rithmic, MT4/5)", tm: true, tz: true, note: "" },
  { feature: "Broker auto-sync (Tradovate, Alpaca, Bybit…)", tm: true, tz: true, note: "" },
  { feature: "AI coach & trade recap", tm: true, tz: true, note: "" },
  { feature: "Pre-trade ritual checklist", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Behavioral tag analysis (FOMO, revenge trade cost)", tm: true, tz: false, note: "TradeMind exclusive" },
  { feature: "Mobile app (iOS / Android PWA)", tm: true, tz: true, note: "" },
  { feature: "Free tier available", tm: true, tz: true, note: "" },
  { feature: "Price (Pro tier)", tm: "$39/mo", tz: "$49/mo", note: "TradeMind is cheaper" },
];

export default function VsTradeZella() {
  return (
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "56px 24px 80px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <h1 style={{ fontSize: "clamp(28px,5vw,48px)", fontWeight: 800, color: "#fff", margin: "0 0 16px", lineHeight: 1.15, letterSpacing: "-1.5px" }}>
            TradeMind vs TradeZella
          </h1>
          <p style={{ fontSize: 18, color: "#a1a1aa", maxWidth: 540, margin: "0 auto 32px", lineHeight: 1.65 }}>
            Both are trading journals. Only one is built specifically for prop firm traders who run multiple funded accounts and need to protect their drawdown.
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 48 }}>
          {[
            { name: "TradeMind", score: "9.4", color: "#00C896", verdict: "Best for prop firm traders", highlights: ["Real-time tilt detection", "Multi-account drawdown tracker", "ICT confluence analytics"] },
            { name: "TradeZella", score: "7.1", color: "#FFB020", verdict: "Good general journal", highlights: ["Solid analytics", "Clean UI", "Good broker integrations"] },
          ].map((p) => (
        ...
        <div style={{ background: "#0d1117", border: "1px solid #1a1f2e", borderRadius: 16, overflow: "hidden", marginBottom: 48 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px", padding: "12px 20px", borderBottom: "1px solid #1a1f2e", background: "#111827" }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: "#71717a", letterSpacing: "0.06em", textAlign: "center" }}>TRADEZELLA</span>
          </div>
          {ROWS.map((row, i) => (
            <div
              key={i}
            ...
            >
              ...
              <div style={{ textAlign: "center" }}>
                {typeof row.tz === "boolean" ? (
                  <span style={{ fontSize: 16, color: row.tz ? "#a1a1aa" : "#ef4444" }}>{row.tz ? "✓" : "✗"}</span>
                ) : (
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#a1a1aa" }}>{row.tz}</span>
                )}
              </div>
            </div>
          ))}
        ...
        {/* Why traders switch */}
        <h2 style={{ fontSize: 22, fontWeight: 700, color: "#fff", margin: "0 0 20px" }}>Why Prop Traders Switch from TradeZella</h2>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 48 }}>
          {[
            { icon: "🚨", title: "Tilt detection", body: "TradeZella shows you your stats after the fact. TradeMind warns you in real time — during your trading session — when your behavior shows signs of revenge trading or tilt." },
            { icon: "🏦", title: "Multi-account risk view", body: "Running FTMO + Apex + TopStep at the same time? TradeMind shows all your drawdown bars side by side. TradeZella has no consolidated prop firm dashboard." },
            { icon: "🔬", title: "Confluence edge", body: "TradeMind is the only journal that shows your win rate for ICT setup combinations — FVG+OB, BOS+OB, etc. You know exactly which combos print money." },
            { icon: "🧠", title: "Mental readiness", body: "Before you risk $5,000 on an FTMO account, TradeMind tells you: GO, CAUTION, or NO-TRADE. TradeZella doesn't assess whether your brain is ready to trade." },
          ].map((item) => (
