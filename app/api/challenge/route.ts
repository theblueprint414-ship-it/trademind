...
export async function GET() {
  ...
  const user = await db.user.findUnique({
    ...
    select: {
      ...
      challengeFirm: true,
      challengeAccountSize: true,
      challengeDailyLimit: true,
      challengeMaxDrawdown: true,
      challengeStartDate: true,
      challengeEndDate: true,
      challengeProfitTarget: true,
      challengeTradingDaysTarget: true,
    },
  });

  if (!user) return Response.json({ error: "Not found" }, { status: 404 });
  if (user.plan !== "premium") return Response.json({ error: "Premium feature" }, { status: 403 });
