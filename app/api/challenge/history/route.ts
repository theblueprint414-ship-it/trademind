...
export async function GET() {
  ...
  try {
    ...
    logger.error("Challenge history GET failed", err, { userId: session.user.id });
    return Response.json({ error: "Failed to fetch challenge history" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error("Challenge history POST failed", err, { userId: session.user.id });
    return Response.json({ error: "Failed to create challenge attempt" }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    logger.error("Challenge history PATCH failed", err, { userId: session.user.id });
    return Response.json({ error: "Failed to update challenge attempt" }, { status: 500 });
  }
}